...
### **Selected Model**
Model B4 with mean imputation and square root transformations has the greatest adjusted R-squared accounting for 32% of the variance in Wins. It is also the model that makes the most intuitive sense in terms of variable coefficient values, which are also all significant barring one. 
...
```{r}
df_evaluation_sqrt$PREDICT_WINS_sqrt = predict(model_sqrt_4, new = df_evaluation_sqrt)
df_evaluation_sqrt$PREDICT_WINS = (df_evaluation_sqrt$PREDICT_WINS_sqrt)*(df_evaluation_sqrt$PREDICT_WINS_sqrt)
```
