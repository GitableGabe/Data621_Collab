...
author: "Alex Khaykin, Melissa Bowman, Gabriel Campos, Jen Abinette"
output:
  pdf_document:
...
### DIAGNOSTIC PLOTS FOR THE FULL MODEL ON ORIGINAL TRAINING DATA
...
p1 <- df_train_with_combo %>% 
  ggplot(aes(y=TARGET_WINS, x=team_H)) +
  geom_point(color="darkgreen") +
  geom_smooth(method = "lm", se=TRUE) +
  labs(title = "WINS BY TEAM H (COMBINED)",
       x="TEAM H (COMBINED)", y="WINS") +
  theme_bw()

p2 <- df_train_with_combo %>% 
  ggplot(aes(y=TARGET_WINS, x=team_BB)) +
  geom_point(color="darkgreen") +
  geom_smooth(method = "lm", se=TRUE) +
  labs(title = "WINS BY TEAM BB (COMBINED)",
       x="TEAM BB (COMBINED)", y="WINS") +
  theme_bw()

p3 <- df_train_with_combo %>% 
  ggplot(aes(y=TARGET_WINS, x=team_SO)) +
  geom_point(color="darkgreen") +
  geom_smooth(method = "lm", se=TRUE) +
  labs(title = "WINS BY TEAM SO (COMBINED)",
       x="TEAM SO (COMBINED)", y="WINS") +
  theme_bw()

p4 <- df_train_with_combo %>% 
  ggplot(aes(y=TARGET_WINS, x=team_HR)) +
  geom_point(color="darkgreen") +
  geom_smooth(method = "lm", se=TRUE) +
