...
Poisson Regression
```{r}
poisson_model <- glm(TARGET ~ LabelAppeal + AcidIndex + STARS, data = df_wine_train, family = poisson)
summary(poisson_model)
```

```{r}
poisson_model_transformed <- glm(TARGET ~ LabelAppeal + AcidIndex + STARS, data = df_wine_train_transformed, family = poisson)
summary(poisson_model_transformed)
```

Negative Binomial Regression
```{r}
neg_binom_model <- glm.nb(TARGET ~ LabelAppeal + AcidIndex + STARS, data = df_wine_train)
summary(neg_binom_model)
```

```{r}
neg_binom_model_transformed <- glm.nb(TARGET ~ LabelAppeal + AcidIndex + STARS, data = df_wine_train_transformed)
summary(neg_binom_model_transformed)
```

Hurdle Poisson Regression
```{r}
hurdle_poisson_model <- hurdle(TARGET ~ LabelAppeal + AcidIndex + STARS, data = df_wine_train_transformed, dist = "poisson")
summary(hurdle_poisson_model)
```

Zero-Inflated Poisson Regression
```{r}
zip_model <- zeroinfl(TARGET ~ LabelAppeal + AcidIndex + STARS | 1, data = df_wine_train_transformed, dist = "poisson")
summary(zip_model)
```

Need to split the df_wine_train_tramsformed into a test and training set to make an The Root Mean Squared Error (RMSE) comparison of all the regressions.  
```{r}
# Assuming df_wine is your complete dataset
#set.seed(100)  # for reproducibility
#n <- nrow(df_wine)
#train_index <- sample(1:n, 0.8 * n)  # 80% for training, 20% for testing
#df_wine_train <- df_wine[train_index, ]
#df_wine_test <- df_wine[-train_index, ]
```
