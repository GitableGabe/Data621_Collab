...
# Apply the Box-Cox transformation
norm_pug_agency_name_freq <- 
  (freq_list^lambda_pug_agency_name-1)/lambda_pug_agency_name
...
(highly_correlated_pairs 
  <- which(cor_matrix_for_model > cor_threshold & cor_matrix_for_model < 1,
           arr.ind = TRUE))
...
```{r}
logistic_model <- glm(Antisemitic_crimes~ norm_ori_freq +
                        norm_state_abbr_freq + norm_data_year_freq +
                        victim_types_freq + division_name_freq  +
                        region_name_freq + population_group_code_freq +
                        offender_race_freq ,df_train_model, family = binomial)
...
```{r}
(probability_predictions <- predict(logistic_model, newdata = df_test_model,
                                    type = "response"))
```
...
```{r}
df_test_model <- cbind(df_test_model, as.data.frame(probability_predictions),
                       as.data.frame(binary_predictions))
```

```{r}
ls_class <- relevel(factor(df_test_model$Antisemitic_crimes),
                    ref = "1") ## changes it from the default ref of 0
...
roc(as.numeric(ls_class), as.numeric(ls_scr_class), plot = TRUE,
    print.auc = TRUE)
```