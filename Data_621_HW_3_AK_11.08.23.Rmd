...
Creating a correlation plot with hierarchical clustered order.
...
Running third model with removed lstat variable.

```{r}
model_3 <- glm(formula = target ~ .-lstat, family = binomial, data = df_crime_train)

summary(model_3)
anova(model_1,model_3)
```
Running model with removed indus variable.

```{r}
model_4 <- glm(formula = target ~ .-lstat-indus, family = binomial, data = df_crime_train)
...
```{r}
model_5 <- glm(formula = target ~ .-lstat-indus-rm, family = binomial, data = df_crime_train)
...
```
```{r}
model_6 <- glm(formula = target ~ .-lstat-indus-rm-chas-dis, family = binomial, data = df_crime_train)

summary(model_6)
anova(step_model,model_6)
```


```{r}
model_final <- glm(formula = target ~ .-lstat-indus-rm-chas-dis-zn, family = binomial, data = df_crime_train)

summary(model_final)
anova(model_final,model_6)
```


```{r}
data_long_2 <- df_crime_train %>% 
  pivot_longer(!target, names_to = "Variable", values_to = "Value") 
data_long_2 %>% 
  filter(Variable!="lstat" | Variable!="indus" | Variable!="rm" | Variable!="chas" | Variable!= "dis" | Variable!="zn") %>%
